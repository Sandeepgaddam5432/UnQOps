...
export const ToolInvocations = memo(({ toolInvocations, toolCallAnnotations, addToolResult }: ToolInvocationsProps) => {
  ...
  return (
    <div className="tool-invocation border border-unqops-elements-borderColor flex flex-col overflow-hidden rounded-lg w-full transition-border duration-150">
      <div className="flex">
        <button
          className="flex items-stretch bg-unqops-elements-artifacts-background hover:bg-unqops-elements-artifacts-backgroundHover w-full overflow-hidden"
          onClick={toggleDetails}
          aria-label={showDetails ? 'Collapse details' : 'Expand details'}
        >
          <div className="p-2.5">
            <div className="i-ph:wrench text-xl text-unqops-elements-textSecondary hover:text-unqops-elements-textPrimary transition-colors"></div>
          </div>
          <div className="border-l border-unqops-elements-borderColor p-2.5 w-full text-left">
            <div className="w-full text-unqops-elements-textPrimary font-medium leading-5 text-sm">
              MCP Tool Invocations{' '}
              {hasToolResults && (
                <span className="w-full w-full text-unqops-elements-textSecondary text-xs mt-0.5">
        ...
        <AnimatePresence>
          {hasToolResults && (
            <motion.button
              ...
              className="bg-unqops-elements-artifacts-background hover:bg-unqops-elements-artifacts-backgroundHover"
              onClick={toggleDetails}
            >
              <div className="p-2">
                <div
                  className={`${showDetails ? 'i-ph:caret-up-bold' : 'i-ph:caret-down-bold'} text-xl text-unqops-elements-textSecondary hover:text-unqops-elements-textPrimary transition-colors`}
                ></div>
              </div>
            </motion.button>
          )}
      ...
      <AnimatePresence>
        {hasToolCalls && (
          ...
          >
            <div className="bg-unqops-elements-artifacts-borderColor h-[1px]" />

            <div className="px-3 py-3 text-left bg-unqops-elements-actions-background">
              <ToolCallsList
        ...
        {hasToolResults && showDetails && (
          ...
          >
            ...
            <div className="p-5 text-left bg-unqops-elements-actions-background">
          ...
          return (
            ...
            >
              <div className="flex items-center gap-1.5 text-xs mb-1">
                {isErrorResult ? (
                  <div className="text-lg text-unqops-elements-icon-error">
                    <div className="i-ph:x"></div>
                  </div>
                ) : (
                  <div className="text-lg text-unqops-elements-icon-success">
                    <div className="i-ph:check"></div>
                  </div>
                )}
                <div className="text-unqops-elements-textSecondary text-xs">Server:</div>
                <div className="text-unqops-elements-textPrimary font-semibold">{annotation?.serverName}</div>
              </div>

              <div className="ml-6 mb-2">
                <div className="text-unqops-elements-textSecondary text-xs mb-1">
                  Tool: <span className="text-unqops-elements-textPrimary font-semibold">{toolName}</span>
                </div>
                <div className="text-unqops-elements-textSecondary text-xs mb-1">
                  Description:{' '}
                  <span className="text-unqops-elements-textPrimary font-semibold">{annotation?.toolDescription}</span>
                </div>
                <div className="text-unqops-elements-textSecondary text-xs mb-1">Parameters:</div>
                ...
                <div className="text-unqops-elements-textSecondary text-xs mt-3 mb-1">Result:</div>
                <div className="bg-[#FAFAFA] dark:bg-[#0A0A0A] p-3 rounded-md">
            ...
            >
              <div className="">
                <div key={toolCallId} className="flex flex-col gap-1">
                  <div className="flex items-center text-unqops-elements-textSecondary font-semibold text-sm">
                    <button
                    ...
                    Calling MCP tool{' '}
                    <span className="ml-0.5 font-light font-mono text-unqops-elements-textPrimary bg-unqops-elements-background-depth-3 px-1.5 py-0.5 rounded-md">
                      {toolName}
                    </span>
                  </div>
                  {expanded[toolCallId] && (
                    <div className="flex gap-3">
                      <div className="w-[0.1px] min-h-[40px] bg-unqops-elements-background-depth-3 ml-1.5" />
                      <div className="flex flex-col gap-1 w-full">
                        <div className="text-unqops-elements-textSecondary text-xs mb-1">
                          Description:{' '}
                          <span className="text-unqops-elements-textPrimary font-semibold">
                            {annotation?.toolDescription}
                          </span>
                        </div>
                        <div className="flex w-full items-stretch space-x-2">
                          <div className="w-full rounded-md bg-unqops-elements-background-depth-3 p-3 ml-0 border-l-2 border-unqops-elements-borderColor">
                            <JsonCodeBlock
                              className="mb-0"
